...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install LocalLama GUI into a repository-local virtual environment."
    )
    parser.add_argument(
        "--venv", default=DEFAULT_VENV, help="Virtual-environment directory (default: .venv)."
    )
    parser.add_argument(
        "--python",
        dest="python_executable",
        help="Python executable for the new virtual environment.",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show planned actions without changing the system."
    )
    parser.add_argument(
        "--no-launcher", action="store_true", help="Skip Unix launcher/desktop integration."
    )
    parser.add_argument(
        "--desktop-entry", action="store_true", help="On Linux, also install the desktop entry."
    )
    parser.add_argument(
        "--windows-desktop", action="store_true", help="Also create a Windows desktop shortcut."
    )
    parser.add_argument(
        "--check-backend",
        nargs="?",
        ...
        metavar="URL",
        help="Check a backend after installation. Defaults to Ollama's local API.",
    )
...
def check_python() -> None:
    ...
    if version < MIN_PYTHON or version > MAX_PYTHON:
        raise SystemExit(
            ...
            f"{version[0]}.{version[1]}. LocalLama GUI supports Python 3.11 through 3.13."
        )
...
def resolve_python(requested: str | None) -> str:
    ...
    if (major, minor) < MIN_PYTHON or (major, minor) > MAX_PYTHON:
        raise SystemExit(
            f"Unsupported Python executable {candidate}: {major}.{minor}. "
...
def ps_quote(value: Path) -> str:
    """Quote a filesystem path for a single-quoted PowerShell string."""
    return str(value).replace("'", "''")


def create_windows_shortcut(
    target_python: Path, root: Path, destination: Path, dry_run: bool
) -> None:
    if not dry_run:
    ...
    target = ps_quote(target_python)
    working_dir = ps_quote(root)
    link = ps_quote(destination)
    script = (
        "$ws = New-Object -ComObject WScript.Shell; "
        f"$s = $ws.CreateShortcut('{link}'); "
        f"$s.TargetPath = '{target}'; "
        "$s.Arguments = '-m locallama_gui'; "
        f"$s.WorkingDirectory = '{working_dir}'; "
        "$s.Description = 'LocalLama Control Center'; "
        "$s.Save()"
    )
    run(
...
def verify_installation(root: Path, venv: Path, dry_run: bool) -> None:
    ...
    run(
        [
            str(python),
            "-c",
            "import locallama_gui; import PySide6; import httpx; "
            "import platformdirs; import pydantic; import psutil; import markdown; "
            "print('imports ok')",
        ],
        cwd=root,
    )
    result = subprocess.run(
        [
            str(python),
            "-c",
            "import importlib.metadata as m; print(m.version('locallama-gui'))",
        ],
        cwd=root,
        capture_output=True,
        text=True,
